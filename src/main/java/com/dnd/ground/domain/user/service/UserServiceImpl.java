    ...
    public HomeResponseDto showHome(String nickname){
        ...
        if (!userRecordOfThisWeek.isEmpty()){
            ...
            userMatrices.forEach(ms -> ms.forEach(m ->
                    userShowMatrices.add(MatrixSetDto.builder().latitude(m.getLatitude()).longitude(m.getLongitude()).build())));
        ...
        notChallenges.forEach(nf -> exerciseRecordRepository.findRecordOfThisWeek(nf.getId())
                .forEach(e -> friendHashMap.put(nf.getNickName(), e.getMatrices()
                        .stream().map(m -> MatrixSetDto.builder().latitude(m.getLatitude()).longitude(m.getLongitude()).build()).collect(Collectors.toSet()))));
        ...
        for (User u : challenges) {
            ...
            recordOfThisWeek.forEach(e -> e.getMatrices().forEach(m -> showMatrices.add(MatrixSetDto.builder().latitude(m.getLatitude()).longitude(m.getLongitude()).build())));
